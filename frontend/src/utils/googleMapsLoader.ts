...
export const loadGoogleMaps = async (options: GoogleMapsLoaderOptions): Promise<void> => {
  ...
  const callbackName = `initMap_${Date.now()}_${Math.random().toString(36).substring(2, 9)}`; // NOSONAR typescript:S2245 - unique callback naming only, not security-sensitive
  ...
  loadPromise = new Promise<void>((resolve, reject) => {
    ...
    script.onerror = (error: string | Event): void => {
      ...
      reject(
        new Error(
          `Failed to load Google Maps API. Please check your API key: ${options.apiKey ? 'Key is set' : 'Key is missing'}`
        )
      );
    };
...
export const isGoogleMapsLoaded = (): boolean => {
  return isLoaded && !!globalThis.google?.maps;
};
