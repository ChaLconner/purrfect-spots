<template>
  <div class="space-y-6">
    <div class="flex justify-between items-center">
      <h1 class="text-3xl font-bold text-brown-900 font-display">{{ t('admin.roles.title') }}</h1>
    </div>
    <p class="text-brown-500">{{ t('admin.roles.subtitle') }}</p>

    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
      <!-- Role List -->
      <div
        class="bg-white rounded-2xl shadow-sm border border-sand-200 overflow-hidden lg:col-span-1"
      >
        <div class="px-6 py-4 border-b border-sand-100 bg-sand-50/50">
          <h3 class="text-lg font-bold text-brown-800">{{ t('admin.roles.list_title') }}</h3>
        </div>
        ...
        >
          <div
            v-for="role in roles"
            :key="role.id"
            class="px-6 py-4 flex justify-between items-center cursor-pointer transition-all hover:bg-sand-50 group"
            :class="{
              'bg-terracotta-50 border-l-4 border-l-terracotta-500': selectedRole?.id === role.id,
              'border-l-4 border-l-transparent': selectedRole?.id !== role.id,
            }"
          ...
          >
            ...
            <div class="flex items-center gap-3 text-brown-500">
              <span
                class="px-2.5 py-0.5 bg-sand-100 text-brown-600 rounded-full text-xs font-bold transition-colors"
      ...
      <div
        v-if="selectedRole"
        class="bg-white rounded-2xl shadow-sm border border-sand-200 lg:col-span-2 overflow-hidden flex flex-col"
      >
        <div
          class="px-6 py-5 border-b border-sand-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4"
        >
          <div class="flex flex-col">
            <h3 class="text-xl font-bold text-brown-900">
              {{ t('admin.roles.permissions_for') }}:
              <span class="text-terracotta-600">{{ selectedRole.name }}</span>
            </h3>
            <div class="mt-2 flex">
              <span
                class="inline-flex items-center gap-1.5 px-2.5 py-1 bg-terracotta-50 text-terracotta-700 text-xs font-bold rounded-lg uppercase tracking-wide"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  class="h-3.5 w-3.5"
          ...
          <div class="flex items-center gap-2 sm:gap-3">
            <div class="relative hidden sm:block">
              <svg
                class="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-brown-400"
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              ...
              <input
                ...
                class="pl-9 pr-4 py-2 w-48 lg:w-64 border border-sand-300 rounded-xl bg-sand-50 focus:bg-white focus:ring-2 focus:ring-terracotta-400 focus:border-terracotta-400 transition-all font-medium text-sm text-brown-800 placeholder-brown-400 outline-none shadow-inner"
              />
            </div>

            <button
              v-if="dirty"
              class="px-4 py-2 border border-sand-300 text-brown-600 rounded-xl hover:bg-sand-50 transition-all font-bold text-sm flex items-center gap-2"
              :disabled="saving"
            ...
            <button
              class="px-5 py-2 bg-terracotta-500 text-white rounded-xl hover:bg-terracotta-600 transition-all shadow-sm hover:shadow-md disabled:opacity-50 flex items-center gap-2 font-bold"
        ...
        <div
          ...
          class="mx-6 mt-4 p-3 bg-amber-50 border border-amber-200 rounded-xl flex items-start gap-3 animate-fade-in shadow-sm"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
        ...
        <!-- Summary Stats -->
        <div class="px-6 py-4 bg-sand-50/50 border-b border-sand-100 flex gap-8">
          <div class="flex flex-col">
            <span class="text-2xl font-bold text-brown-900">{{ activePermissions.length }}</span>
            <span class="text-xs font-bold text-brown-500 uppercase tracking-widest">{{
              t('admin.roles.assigned')
            }}</span>
          </div>
          <div class="flex flex-col">
            <span class="text-2xl font-bold text-brown-900">{{ totalPermissions }}</span>
            <span class="text-xs font-bold text-brown-500 uppercase tracking-widest">{{
              t('admin.roles.total')
            }}</span>
          </div>
        ...
        <div class="p-6 overflow-y-auto space-y-8 flex-1">
          <div
            ...
            class="space-y-4"
          >
            <div class="flex justify-between items-center pb-2 border-b-2 border-sand-100">
              <div class="flex flex-col gap-1">
                <h4 class="text-lg font-bold text-brown-900 flex items-center gap-2">
                  {{
                  ...
                  <span
                    class="px-2 py-0.5 bg-sand-100 text-brown-600 rounded-full text-xs shrink-0"
                  >
                ...
                <div class="flex items-center gap-2 text-xs font-bold">
                  <button
                    class="text-terracotta-500 hover:text-terracotta-700 hover:underline"
                  ...
                  <button
                    class="text-brown-500 hover:text-brown-700 hover:underline"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
              <label
                ...
                :key="perm.id"
                class="flex items-start gap-3 p-4 rounded-xl border cursor-pointer transition-all hover:-translate-y-0.5"
                :class="
                  activePermissions.includes(perm.id)
                    ? 'bg-terracotta-50/50 border-terracotta-300 shadow-sm'
                    : 'bg-white border-sand-200 hover:border-terracotta-200 hover:bg-sand-50/50'
                "
      ...
      <div
        v-else
        class="bg-white rounded-2xl shadow-sm border border-sand-200 border-dashed lg:col-span-2 flex flex-col items-center justify-center py-24 text-center px-4"
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          class="h-20 w-20 text-sand-300 mb-6"
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
        >
        ...
        <h3 class="text-2xl font-bold text-brown-800 mb-2 font-display">
          {{ t('admin.roles.select_prompt') }}
        </h3>
        <p class="text-brown-500 max-w-sm">{{ t('admin.roles.select_hint') }}</p>
      </div>
    </div>
