    ...
    >
      ...
      <div class="p-6 sm:p-8 md:p-12 flex flex-col justify-center">
        <div class="text-center mb-8">
          <h1 class="font-heading text-[1.6rem] md:text-4xl font-extrabold text-brown-text mb-2">
            {{ isLogin ? $t('auth.welcomeBack') : $t('auth.joinUs') }}
          </h1>
          <p class="font-body text-[0.95rem] text-brown-text">
        ...
        <form class="flex flex-col gap-5" novalidate @submit.prevent="throttledSubmit">
          ...
          <div class="flex flex-col gap-2">
            ...
            <div v-if="isLogin" class="text-right mt-1">
              <router-link
                to="/forgot-password"
                class="font-body text-[0.85rem] text-sage no-underline font-medium hover:underline"
              >
        ...
        <div class="flex items-center gap-4 my-6">
          <span
            class="flex-1 h-px bg-gradient-to-r from-transparent via-sage-pill to-transparent opacity-30"
          ></span>
          <span class="font-body text-[0.85rem] text-sand-500 whitespace-nowrap">{{
        ...
        <BaseButton
          ...
          block
          size="lg"
          class="bg-white border-2 border-sage-light text-brown shadow-sm hover:bg-white hover:border-sage hover:shadow-md transition-all"
        ...
        <!-- Switch Mode Link -->
        <div class="text-center mt-6 font-body text-[0.9rem]">
          <span class="text-brown-text">
            {{ isLogin ? $t('auth.dontHaveAccount') : $t('auth.alreadyHaveAccount') }}
          </span>
          <router-link
            :to="isLogin ? '/register' : '/login'"
            class="text-terracotta font-bold no-underline ml-1 transition-colors duration-200 hover:text-sage-dark hover:underline"
          >
