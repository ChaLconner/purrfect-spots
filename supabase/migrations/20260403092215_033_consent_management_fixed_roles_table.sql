-- Migration: 033_consent_management_fixed_roles_table.sql
-- Description: สร้างตารางสำหรับ Consent Management และ Breach Notification Systems
-- ปรับปรุงให้รองรับโครงสร้างตาราง roles ที่มีอยู่จริง
...
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='users' AND column_name='tos_accepted') THEN
        ALTER TABLE users ADD COLUMN tos_accepted BOOLEAN DEFAULT FALSE;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='users' AND column_name='privacy_accepted') THEN
...
DROP POLICY IF EXISTS "Consent versions are publicly readable" ON consent_versions;
...
-- User consents: admin can read all (fixed to check roles table)
...
DROP POLICY IF EXISTS "Admins can manage incident affected users" ON incident_affected_users;
...
DROP POLICY IF EXISTS "Admins can manage incident timeline" ON incident_timeline;
