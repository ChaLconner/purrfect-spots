...
class TestGalleryServiceExtended:
    ...
    def test_fulltext_search_rpc_success(self, gallery_service, mock_supabase):
    ...
    def test_fulltext_search_rpc_fail_fallback_direct(self, gallery_service, mock_supabase):
        ...
        mock_supabase.textSearch.return_value.order.return_value.limit.return_value.execute.return_value = MagicMock(data=[{"id": "2"}])
    ...
    def test_fulltext_all_fail_fallback_ilike(self, gallery_service, mock_supabase):
        ...
        with patch.object(gallery_service, "_fulltext_search", side_effect=Exception("FT fail")):
            mock_supabase.or_.return_value.order.return_value.limit.return_value.execute.return_value = MagicMock(data=[{"id": "3"}])
    ...
    def test_get_nearby_photos_bbox(self, gallery_service, mock_supabase):
    ...
    def test_get_nearby_photos_postgis(self, gallery_service, mock_supabase):
    ...
    def test_get_nearby_photos_postgis_fail(self, gallery_service, mock_supabase):
        with patch("services.feature_flags.FeatureFlagService.is_enabled", return_value=True):
    ...
    def test_get_photo_by_id_found(self, gallery_service, mock_supabase):
        mock_supabase.single.return_value.execute.return_value = MagicMock(data={"id": "p1"})
        
        photo = gallery_service.get_photo_by_id("p1")
        assert photo["id"] == "p1"

    def test_get_photo_by_id_not_found_rows(self, gallery_service, mock_supabase):
        # Supabase raises exception when single() returns no rows
        mock_supabase.single.return_value.execute.side_effect = Exception("JSON object requested, multiple (or no) rows returned")
        
        photo = gallery_service.get_photo_by_id("p1")
        assert photo is None

    def test_get_photo_by_id_error(self, gallery_service, mock_supabase):
