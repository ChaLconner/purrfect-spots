    ...
    with patch("utils.async_client.async_supabase") as mock_async:
        ...
        with (
            ...
            mock_async.rpc.assert_called_with(
                "toggle_photo_like", {"p_user_id": "user1", "p_photo_id": "photo1"}, jwt_token="token"
            )
...
async def test_get_comments(social_service):
    ...
    with patch("utils.async_client.async_supabase") as mock_async:
        mock_async.select = AsyncMock(
            return_value=[
                {"id": "c1", "content": "cute", "user_id": "user1", "users": {"name": "Alice", "picture": "alice.jpg"}}
            ]
        )
...
async def test_delete_comment_success(social_service):
    ...
    with patch("utils.async_client.async_supabase") as mock_async:
        ...
        mock_async.delete.assert_called_with(
            table="photo_comments", filters={"id": "eq.c1", "user_id": "eq.user1"}, jwt_token="token"
        )
