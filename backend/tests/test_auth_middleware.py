...
async def test_get_jwks_success(mock_env, mock_jwks_response):
    with patch("httpx.AsyncClient.get", new_callable=AsyncMock) as mock_get:
        ...
        # Reset cache globals
        with patch("middleware.auth_middleware._jwks_cache", None), \
...
async def test_get_jwks_cached(mock_env, mock_jwks_response):
    with patch("httpx.AsyncClient.get", new_callable=AsyncMock) as mock_get:
        # Pre-set cache
        with patch("middleware.auth_middleware._jwks_cache", mock_jwks_response), \
...
async def test_verify_and_decode_token_fallback_custom(mock_env):
    with patch("middleware.auth_middleware.decode_supabase_token", side_effect=ValueError), \
...
async def test_get_current_user_from_request_success(mock_env):
    ...
    with patch("middleware.auth_middleware._verify_and_decode_token", new_callable=AsyncMock) as mock_verify, \
