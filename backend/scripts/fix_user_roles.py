...
async def fix_roles():
    ...
    # 2. Target admins
    admin_emails = ["chaluntonvipusanapas@gmail.com", "professional_fes@hotmail.com"]

    print(f"Setting {admin_emails} as admin...")
    for email in admin_emails:
        admin.table("users").update({"role_id": roles["admin"]}).eq("email", email).execute()

    # 3. Set everyone else to user if they don't have a role
    print("Setting default role for others...")
    admin.table("users").update({"role_id": roles["user"]}).is_("role_id", "null").execute()
