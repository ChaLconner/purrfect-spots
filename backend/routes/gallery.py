...
async def get_gallery(
    ...
    try:
        ...
        logger.error(f"Gallery fetch error: {str(e)}", exc_info=True)
        raise HTTPException(
            status_code=500, detail="Failed to fetch gallery images"
        )
...
async def get_all_gallery(
    request: Request,  # Required for rate limiting
    limit: int = Query(1000, ge=1, le=5000, description="Maximum number of images to return (default: 1000)"),
    gallery_service: GalleryService = Depends(get_gallery_service)
):
    """
...
async def get_locations(gallery_service: GalleryService = Depends(get_gallery_service)):
    ...
    try:
        ...
        logger.error(f"Locations fetch error: {str(e)}", exc_info=True)
        raise HTTPException(
            status_code=500, detail="Failed to fetch cat locations"
        )
...
async def get_locations_in_viewport(
    ...
    try:
        ...
        logger.error(f"Viewport fetch error: {str(e)}", exc_info=True)
        raise HTTPException(
...
async def search_locations(
    request: Request,  # Required for rate limiting
    q: Optional[str] = Query(None, description="Text to search in location name and description"),
    ...
    try:
        ...
        photos = gallery_service.search_photos(
            query=q,
            tags=tag_list,
            limit=limit
        )
        ...
        raise HTTPException(
            status_code=500, detail="Failed to search locations"
        )
...
async def get_popular_tags(
    ...
    try:
        ...
        logger.error(f"Popular tags error: {str(e)}", exc_info=True)
        raise HTTPException(
            status_code=500, detail="Failed to fetch popular tags"
        )
