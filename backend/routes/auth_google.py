...
async def google_login_redirect():
    ...
    try:
        ...
        auth_url = (
            f"https://accounts.google.com/o/oauth2/v2/auth?{urlencode(oauth_params)}"
        )
    ...
    try:
        ...
        if not redirect_origin and request.redirect_uri.endswith("/auth/callback"):
            ...
            if len(uri_parts) >= 3:
                origin = "/".join(
                    uri_parts[:3]
                )  # https://domain.com or http://localhost:5173
        ...
        if not redirect_origin:
            print(
                f"Auth Exchange Debug: No match found. Expected one of: {expected_redirects}"
            )
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Code exchange failed: {e!s}",
        )
