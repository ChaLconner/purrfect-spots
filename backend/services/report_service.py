...
class ReportService:
    ...
    async def create_report(self, photo_id: str, reporter_id: str, reason: str, details: str | None = None) -> dict[str, Any]:
        """
    ...
    async def get_user_reports(self, user_id: str) -> list[dict[str, Any]]:
        ...
        """
        try:
            res = await self.supabase.table("reports").select("*").eq("reporter_id", user_id).order("created_at", desc=True).execute()
            return res.data or []
        except Exception as e:
