...
- **Commits**: Conventional (`feat:`, `fix:`, `docs:`, `refactor:`, `chore:`). See [GIT_GUIDELINES.md](./GIT_GUIDELINES.md) for full details.

- **Branches**: `main` (prod), `dev` (integration), `feature/*`, `fix/*`. See [GIT_GUIDELINES.md](./GIT_GUIDELINES.md).

- **Pre-commit**: Lint/Type-check must pass. Local hooks via Husky.

- **Pull Requests**: Required for `main` and `dev`. No direct pushes. Must be linked to an Issue.
